...
function expireOldPayments(){
  ...
  payments.forEach(p=>{
    ...
    // Only CLAIMED orders get a 15-minute expiry.
    // ACTIVE orders stay available until admin removes them.
    if(
      !p.expires_at &&
      p.status==="claimed" &&
      p.claimed_at
    ){
      const baseTime=new Date(p.claimed_at).getTime();
...
app.get("/api/admin/dashboard",adminAuth,(req,res)=>{
  try{
    ...
    const pendingWithdrawals=transactions.filter(t=>
      t.type==="upi_withdraw" && t.status==="pending"
    ).length;

    const totalDeposits=transactions
      .filter(t=>["payment_deposit","usdt_deposit"].includes(t.type))
      .reduce((sum,t)=>sum+Number(t.amount||0),0);

    const totalWithdrawals=transactions
      .filter(t=>["upi_withdraw","admin_withdraw"].includes(t.type))
      .reduce((sum,t)=>sum+Number(t.amount||0),0);

    const pendingUSDT=readUSDT().deposits.filter(d=>d.status==="pending").length;
...
/* ADMIN REMOVE UNCLAIMED PAYMENT ORDER */

app.delete("/api/admin/payments/:id",adminAuth,(req,res)=>{
  try{
    ...
    if(payment.status!=="active" || payment.user_id){
      return res.status(400).json({
        error:"Only unclaimed active orders can be removed"
      });
    }
    ...
    res.json({
      ok:true,
      message:"Payment order removed successfully"
    });

  }catch(e){

    console.error("Admin remove payment error:",e);

    res.status(500).json({
      error:"Could not remove payment order"
    });
...
app.get("/api/admin/transactions",adminAuth,(req,res)=>{
  try{
    ...
    res.json({ok:true,transactions:transactions.slice(0,500)});
  }catch(e){
    console.error(e);
...
/* ADMIN TEAM / REFERRAL OVERVIEW */
app.get("/api/admin/team",adminAuth,(req,res)=>{
  try{
    const users=readUsers();
    const transactions=readTransactions();
    const list=users.map(u=>{
      const direct=users.filter(x=>String(x.referrer_id||"")===String(u.id)).length;
      const deposits=transactions
        .filter(t=>t.user_id===u.id && t.type==="payment_deposit")
        .reduce((sum,t)=>sum+Number(t.amount||0),0);
      return {
        id:u.id, username:u.username, referral_code:u.referral_code||"",
        referrer_id:u.referrer_id||null, direct_members:direct,
        total_deposit:Number(deposits.toFixed(2)), created_at:u.created_at
      };
    });
    res.json({ok:true,users:list});
  }catch(e){
    console.error(e);
    res.status(500).json({error:"Could not load team data"});
  }
});
...
app.post("/api/usdt/deposit",auth,(req,res)=>{
  try{
    ...
    // Maximum 5 pending USDT deposits per user.
    // Approved/rejected deposits are not counted.
    const pendingCount=data.deposits.filter(d=>
    ...
    if(pendingCount>=5){
      return res.status(400).json({
        error:"Aapke 5 USDT deposit requests already pending hain. Pehle kisi request ka approval ya rejection hone dein."
      });
    }
...
app.get("/api/statistics",auth,(req,res)=>{
  try{
    ...
    // Today's Sell:
    // successful UPI withdrawals + Admin Remove Balance
    ...
    // Admin Add Balance is also counted as today's Deposit
    ...
    // Total Deposit = approved payment + approved USDT + Admin Add Balance
    const deposit=paymentDeposit+usdtDeposit+adminDeposit;
    ...
    // Live referral commission (B + C levels)
